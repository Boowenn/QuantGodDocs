...
- EA currently evaluates `RSI_Reversal`, `MA_Cross`, `BB_Triple`, `MACD_Divergence`, `SR_Breakout`, `USDJPY_TOKYO_RANGE_BREAKOUT`, `USDJPY_NIGHT_REVERSION_SAFE`, and `USDJPY_H4_TREND_PULLBACK` contracts in the shadow/tester/paper lane; unsupported families remain adapter-gap evidence for Case Memory and GA;
